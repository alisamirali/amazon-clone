...
const Header = () => {
  ...
  const { products, favoriteData, userInfo, allProducts } = useSelector(
  ...
  return (
    <div className="w-full h-20 bg-amazon_blue text-lightText sticky top-0 z-50">
      <div className="h-full w-full mx-auto inline-flex items-center justify-between gap-1 mdl:gap-3 px-4">
        ...
        >
          <Image className="w-28 object-contain" src={logo} alt="logoImg" />
        </Link>
        {/* delivery */}
        <div className="px-2 border border-transparent hover:border-white cursor-pointer duration-300 items-center justify-center h-[70%] hidden xl:inline-flex gap-1">
          ...
          <div className="text-xs">
            <p>Deliver to</p>
            <p className="text-white font-bold uppercase">EG</p>
          </div>
        </div>
        {/* search bar */}
        <div className="flex-1 h-10 hidden md:inline-flex items-center justify-between relative">
          ...
          {/* ========== Searchfield ========== */}
          {/* {searchQuery && (
            <div className="absolute left-0 top-12 w-full mx-auto max-h-96 bg-gray-200 rounded-lg overflow-y-auto cursor-pointer text-black">
        ...
        <Link
          href={"/cart"}
          className="hidden md:flex items-center px-2 border border-transparent hover:border-white cursor-pointer duration-300 h-[70%] relative"
        >
          <Image
        ...
        {/* menu */}
        <span className="flex md:hidden text-gray-100 border border-transparent hover:border-white cursor-pointer p-2">
          <RxHamburgerMenu size={25} />
        </span>
      </div>

      {/* <div className="absolute md:hidden top-0 l-0 w-[100%] h-screen bg-gray-50">
        <div className="w-full flex items-center justify-end p-3 mb-3">
          <GrClose size={25} />
        </div>

        <ul className="list-none p-2 flex flex-col gap-4 justify-start">
          {userInfo ? (
            <li className="flex items-center border border-transparent hover:border-white cursor-pointer duration-300 h-[70%] gap-1">
              <Image
                src={userInfo.image}
                alt="userImage"
                className="w-8 h-8 rounded-full object-cover"
          ...
          <li className="text-black text-lg font-semibold flex items-center">
